...
public struct Tweet : CustomStringConvertible
{
    ...
    fileprivate static func mediaItems(from twitterData: NSArray?) -> [MediaItem] {
    ...
    fileprivate static func mentions(from twitterData: NSArray?, in text: String, with prefix: String) -> [Mention] {
...
public struct Mention: CustomStringConvertible
{
    ...
    init?(from data: NSDictionary?, in text: NSString, with prefix: String)
    {
        guard
            ...
            let start = (indices.firstObject as? NSNumber)?.int32Value, start >= 0,
            let end = (indices.lastObject as? NSNumber)?.int32Value, end > start
        else {
            return nil
        }
