...
//        }
    ...
    open func fetch(_ handler: @escaping (PropertyList?) -> Void) {
    ...
    open var older: Request? {
        if min_id == nil {
            if parameters[TwitterKey.maxID] != nil {
                return self
            }
        } else {
            return modifiedRequest([TwitterKey.maxID : min_id!])
        }
        return nil
    }
    ...
    open var newer: Request? {
        if max_id == nil {
            if parameters[TwitterKey.sinceID] != nil {
                return self
            }
        } else {
            return modifiedRequest([TwitterKey.sinceID : max_id!], clearCount: true)
        }
        return nil
    }
    ...
    fileprivate func modifiedRequest(_ parametersToChange: Dictionary<String,String>, clearCount: Bool = false) -> Request {
    ...
    fileprivate func log(_ whatToLog: Any) {
